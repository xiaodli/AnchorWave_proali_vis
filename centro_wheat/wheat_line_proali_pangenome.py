...
class Line:
    def __init__(self, config_pra):
        ...
        self.centro = config_pra['line']['centro']
    ...
    @staticmethod
    def line_get_centro_pos_list(centro_df, chr_to_start):
        start_list = []
        end_list = []
        for i, row in centro_df.iterrows():


            new_start = chr_to_start[row['chr']] + row['Start']
            new_end = chr_to_start[row['chr']] + row['End']
            start_list.append(new_start)
            end_list.append(new_end)
        return start_list, end_list

    @staticmethod
    def read_centro(centro_file):
        df = pd.read_csv(centro_file, header=0, sep='\t')
        df['Sample'] = df['Sample'].astype(str)
        df['chr'] = df['chr'].astype(str)
        df['Start'] = df['Start'].astype(int)
        df['End'] = df['End'].astype(int)
        df['chr'] = df['Sample'] + df['chr']
        return df
    ...
    def read_proali_collinearity(qry_prefix, ref_prefix, collinearity, chr_list, chr_to_start):
                    ...
                    if flag:
                        ...
                        if ref_prefix + chr_pair[0] not in chr_list or qry_prefix + chr_pair[1] not in chr_list:
                            ...
                            if flag_number:
                                ...
                                block_direction.append(line.split()[6])
                                block_index += 1
                                flag_number = False
                        if flag:
        ...
        return data, ref_chr_list, query_chr_list, block_direction
    ...
    @staticmethod
    def plot_centro_chr(start, end, chr_width, height):
        x = []
        y = []
    ...
    def sub_run(self, collinearity, prefix, ref_length, query_length, total_length, query_height, ref_height, loop, last_loop, strip_chr_abbr, radian, change_length, margin_radius, ref_centro, query_centro):
        ...
        ref_centro_start_list, ref_centro_end_list = self.line_get_centro_pos_list(ref_centro, ref_chr_to_start)
        centro_chr_pos_zip = zip(ref_chr_list, ref_centro_start_list, ref_centro_end_list)
        centro_chr_pos_map = {}
        for ch, start, end in centro_chr_pos_zip:
            start_x = start / total_length
            end_x = end / total_length
            centro_chr_pos_map[ch] = [start_x, end_x]
        ...
        for i in range(len(ref_chr_list)):
            ...
            # modify outline?
            if loop in [0, 1, 2, 3 ,4, 5]:
                plt.fill(x, y, facecolor='#E83828', alpha=1, zorder=3)
            elif loop in [6, 7, 8]:
                plt.fill(x, y, facecolor='#E78A00', alpha=1, zorder=3)
            else:
                plt.fill(x, y, facecolor='#3F40C3', alpha=1, zorder=3)
            ...
            x_centro, y_centro = self.plot_centro_chr(centro_chr_pos_map[ref_chr_list[i]][0], centro_chr_pos_map[ref_chr_list[i]][1], self.width, ref_height)
            # plt.fill(x_centro, y_centro, facecolor='#9FA0A0', alpha=0.7, zorder=3.5)
            center_x = (centro_chr_pos_map[ref_chr_list[i]][0] + centro_chr_pos_map[ref_chr_list[i]][1])/2
            center_y = ref_height + self.width / 2
            plt.scatter(center_x, center_y, s=18.5, linewidths=0 ,color='#9FA0A0', alpha=0.9, zorder=3.5)
        ...
        query_centro_start_list, query_centro_end_list = self.line_get_centro_pos_list(query_centro, query_chr_to_start)
        centro_chr_pos_zip = zip(query_chr_list, query_centro_start_list, query_centro_end_list)
        centro_chr_pos_map = {}
        for ch, start, end in centro_chr_pos_zip:
            start_x = start / total_length
            end_x = end / total_length
        ...
        if loop == last_loop:
            ...
            for i in range(len(query_chr_list)):
                ...
                # modify outline ?
                plt.fill(x, y, facecolor='#3F40C3', alpha=1)
                ...
                for abbr in strip_chr_abbr:
                    if text_chr.startswith(abbr):
                        text_chr = text_chr[len(abbr):]
                # plt.text(label_x, label_y, text_chr, ha="center", va="center", fontsize=self.font_size, color='black')
                x_centro, y_centro = self.plot_centro_chr(centro_chr_pos_map[query_chr_list[i]][0],
                                                          centro_chr_pos_map[query_chr_list[i]][1], self.width,
                                                          query_height)
                # plt.fill(x_centro, y_centro, facecolor='#9FA0A0', alpha=0.7, zorder=3.5)
                center_x = (centro_chr_pos_map[query_chr_list[i]][0] + centro_chr_pos_map[query_chr_list[i]][1]) / 2
                center_y = query_height + self.width / 2
                plt.scatter(center_x, center_y, s=18.5, linewidths = 0, color='#9FA0A0', alpha=0.9, zorder=3.5)
        # print(ref_chr_list)
        ...
        data, rf_blk_chr, qry_blk_chr, block_direction = self.read_proali_collinearity(prefix[1], prefix[0], collinearity, chr_list, chr_to_start)
        intra = []
        intra_chr_list = []
        i = 0
        # intra_color_index = []
        for block in data:
            if qry_blk_chr[i] == rf_blk_chr[i]:
                ...
                plt.fill(x, y, facecolor=color, alpha=0.8, zorder=zorder_val)
                i += 1
        assert len(intra) == 0
        i = 0
        for block in intra:
    ...
    def run(self):
        ...
        for le in length_file_list:
            ...
            if len(le) == 0:
                continue
            length = pd.read_csv(le, sep='\t', header=None)
            length.columns = ['chr', 'length', 'three', 'four', 'five']
            # print(length)
        ...
        # split centro get dataframe
        centro_file_list = self.centro.split(",")
        strip_centro_file_list_df = []
        new_centro_file_list_df = []

        i = 0
        for le in centro_file_list:
            le = le.strip()
            if len(le) == 0:
                continue
            centro_df = self.read_centro(le)
            strip_centro_file_list_df.append(centro_df)
            i += 1

        for i in range(len(strip_centro_file_list_df)-1):
            new_centro_file_list_df.append([strip_centro_file_list_df[i], strip_centro_file_list_df[i+1]])
        ...
        zipped_four_pair = list(zip(strip_collinearity_file_list, new_prefix, new_length_file_list_df, new_centro_file_list_df))
        ...
        last_loop = len(zipped_four_pair) - 1
        i = 0
        for col, prefix, length, centro_pair in zipped_four_pair:


            self.sub_run(col, prefix, length[0], length[1], total_length, query_height, ref_height, i, last_loop, strip_chr_abbr, radian, change_length, margin_radius, centro_pair[0], centro_pair[1])
